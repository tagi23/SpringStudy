...
public class Caculator{
    ...
    public static int calculator(positiveNumber operand_1 , String operator , positiveNumber operand_2) {
        // return ArithmeticOperator.calculator(operand_1 , operator , operand_2);
        return arithmeticOperator.stream()
                .filter(arithmeticOperator ->arithmeticOperator.supports(operator))  //올바른 operator를 찾는다.
                .map(arithmeticOperator -> arithmeticOperator.calculate(operand_1,operand_2))//찾은 구현체에게 calculate를 위임한다 (int로 받기위해 map을 해줌)
                .findFirst()
